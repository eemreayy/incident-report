...
| **TC-10** | SSE bağlantı yönetimi (sunucu) | Timeout, koptuğunda yeniden bağlanma, çok istemcili yayın |
...
| **TC-13** | **Canlı akışta agregasyon tazeleme stratejisi** | Yeni kayıt geldiğinde grafik ve özet **yeniden sorgulanacak** mı, yoksa delta istemcide mi uygulanacak? Yeniden sorgu her zaman doğru ama sık olayda gürültülü; delta hızlı ama kümülatif/agrega kuralını istemcide kopyalar (NFR-13'e aykırı). Ayrıca aktif filtreye uymayan olay ne yapmalı? |
| **TC-14** | **`SHARED` ve `UNKNOWN` kapsamın arayüzde temsili** | Tabloda ve grafikte bölüştürmeden, düşürmeden, çift saymadan gösterim; il toplamları ile genel toplamın okuyucu tarafından uzlaştırılabilmesi (ADR-019). Grafikte ayrı seri mi, ayrı gösterge mi? |
| **TC-15** | **İstemci durumu ile sunucu durumunun ayrımı** | Filtre durumunun adres çubuğuyla senkronizasyonu; sunucu verisi önbelleğinin canlı akış geldiğinde nasıl geçersizleştirileceği; grafik/özet/liste arasında **tek** filtre kaynağı |
| **TC-16** | **Anlamlı %80 kapsam (frontend)** | Sayıyı render testleriyle şişirmeden gerçek davranışı ölçmek; SSE, grafik ve zamanlama içeren kodun deterministik test edilmesi (sahte zaman, sahte akış) |
...
| **TC-18** | **Anahtar kelime vurgulamanın hizalanması** | Vurgulama ham metnin **değiştirilmemiş** hali üzerinde yapılır (FR-02); backend'in ürettiği offset'ler Unicode/Türkçe karakterlerde ve HTML'e yerleştirmede kaymamalı (C-3) |
